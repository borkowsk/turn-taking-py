...
class Metrics:
    ...
    def user_ftime_ranking(self) -> pd.Series:
        time = self.nbdata.groupby("tier", observed=False)["duration"] \
            .sum() \
    ...
    def user_unit_ranking(self) -> pd.Series:
        units = self.nbdata.groupby("tier", observed=False) \
            .size() \
            .sort_values(ascending=False)
    ...
    def dump(
        ...
        if mode == "global":
            ...
            out = out.groupby("tier", observed=False).first()
        else:
            sdata = self.sdata
    ...
    def tt_equality(self) -> float:
        return self.entropy(self.nbdata.groupby("tier", observed=False)["duration"].sum()) / 2
    @Meta.metric("global")
    ...
    def u_time_talk(self) -> pd.Series:
        return self.data.groupby("tier", observed=False)["talk_time"].sum()
    @Meta.metric("user")
    ...
    def u_mean_time(self) -> pd.Series:
        ...
        return df.groupby("tier", observed=False)["talk_time"].mean()
    ...
    def u_pause_mean_time(self) -> pd.Series:
        df = self.data[self.data["event"] == "continuation"]
        return df.groupby("tier", observed=False)["gap_time"].mean()
